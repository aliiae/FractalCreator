...
public:
  Area(T XMin, T XMax, T YMin, T YMax)
  ...
  void setXMin(T XMin) { x_min = XMin; }
  T getXMax() const { return x_max; }
  void setXMax(T XMax) { x_max = XMax; }
  T getYMin() const { return y_min; }
  void setYMin(T YMin) { y_min = YMin; }
  T getYMax() const { return y_max; }
